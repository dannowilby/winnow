...
data "aws_ami" "hadoop" {
  ...
  filter {
    name   = "name"
    values = ["hadoop-benchmark-*"]
  }

  owners = ["self"]
}

resource "aws_instance" "benchmark-control-plane" {
  ...
  user_data = file("start-namenode.sh")
  ...
  tags = {
    Name = "Benchmark control plane"
  }
}

output "control-plane-id" {
  description = "Identifier for the Benchmark control plane"
  value       = aws_instance.benchmark-control-plane.id
}
...
resource "aws_instance" "benchmark-worker" {
  ...
  user_data = file("start-datanode.sh")
...
data "aws_iam_policy_document" "eice-connect" {
  ...
  statement {
    ...
    resources = [aws_instance.benchmark-control-plane.arn]
