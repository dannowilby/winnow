...
fn load_cluster() -> Result<(ClusterList, (IpAddr, u16))> {
    ...
    let loopback = match std::env::var("PORT").ok().and_then(|p| p.parse::<u16>().ok()) {
        Some(port) => config
            .members
            .iter()
    ...
    Ok((ClusterList::from_hosts(config.members, loopback), server_addr))
}
...
async fn main() -> Result<()> {
    ...
    listener
        ...
        .map(|channel| async {
            let server =
                MapReduceServer::new(cluster.clone(), job_lookup.clone(), wasm_env.clone());
